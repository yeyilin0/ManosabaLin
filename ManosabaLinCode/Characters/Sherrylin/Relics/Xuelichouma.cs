...
public sealed class Xuelichouma : ManosabaRelicTemplate, IEasyRightClickableRelic
{
    ...
    [
        "event:/ManosabaLin/sfx/relics/xueli0",
        "event:/ManosabaLin/sfx/relics/xueli1",
        "event:/ManosabaLin/sfx/relics/xueli2",
        "event:/ManosabaLin/sfx/relics/xueli3",
        "event:/ManosabaLin/sfx/relics/xueli4",
        "event:/ManosabaLin/sfx/relics/xueli5",
        "event:/ManosabaLin/sfx/relics/xueli6"
    ];
    ...
    public async Task OnRightClick(PlayerChoiceContext choiceContext, RightClickContext clickContext)
    {
        ...
        var sherrylinPool = ModelDb.GetById<CardPoolModel>(
            ModelDb.GetId(typeof(SherrylinCardPool)));

        var unlockedCards = sherrylinPool
        ...
        switch (roll)
        {
            case 0: // 稀有 + 0费
                ...
                if (rareZeroCost.Count > 0)
                {
                    if (!isInCombat)
                    {
                        await OfferOutOfCombatCardReward(
                            rareZeroCost,
                            [sherrylinPool],
                ...
                if (rareCards.Count > 0)
                {
                    if (!isInCombat)
                    {
                        await OfferOutOfCombatCardReward(
                            rareCards,
                            [sherrylinPool],
                            c => c.Rarity == CardRarity.Rare && c.CanBeGeneratedInCombat,
                            rng);
                        return;
                    }
                ...
                if (uncommonCards.Count > 0)
                {
                    if (!isInCombat)
                    {
                        await OfferOutOfCombatCardReward(
                            uncommonCards,
                            [sherrylinPool],
                            c => c.Rarity == CardRarity.Uncommon && c.CanBeGeneratedInCombat,
                            rng);
                        return;
                    }
                ...
                if (commonCards.Count > 0)
                {
                    if (!isInCombat)
                    {
                        await OfferOutOfCombatCardReward(
                            commonCards,
                            [sherrylinPool],
                            c => c.Rarity == CardRarity.Common && c.CanBeGeneratedInCombat,
                            rng);
                        return;
                    }
            ...
            case 5: // 魔女卡检查
                var deckCards = isInCombat
                    ? Owner.Piles.SelectMany(p => p.Cards)
                ...
                if (existingWitchCard != null)
                {
                    if (!isInCombat)
                    {
                        var allPools = Owner.UnlockState.CharacterCardPools.ToArray();
                        var allAncient = allPools
                        ...
                        var offered = await OfferOutOfCombatCardReward(
                            allAncient,
                            allPools,
                            c => c.Rarity == CardRarity.Ancient,
                            rng);
                        if (offered) Owner.Gold = 0;
                        return;
                    }

                    generatedCard = RollAncientFromAllPools(rng, isInCombat);
                    if (isInCombat)
                        combatCard = Owner.Creature.CombatState.CreateCard(generatedCard.CanonicalInstance, Owner);
                }
                else
                {
                    if (!isInCombat)
                    {
                        var allPools = Owner.UnlockState.CharacterCardPools.ToArray();
                        var witchCards = WitchPowerCardIds
                            .Select(id => ModelDb.GetById<CardModel>(new ModelId("CARD", id)))
                            .Where(c => c != null)
                            .ToList();
                        ...
                        var offered = await OfferOutOfCombatCardReward(
                            witchCards,
                            allPools,
                            c => WitchPowerCardIds.Contains(c.Id.Entry),
                            rng);
                        if (offered) Owner.Gold = 0;
                        return;
                    }
            ...
            case 6: // 其他角色卡池（不含先古）
                ...
                if (otherPools.Count > 0)
                {
                    if (!isInCombat)
                    {
                        var rewardPools = Owner.UnlockState.CharacterCardPools
                            .Where(p => p != Owner.Character.CardPool)
                            .ToArray();
                        await OfferOutOfCombatCardReward(
                            otherPools,
        ...
        if (isInCombat)
        {
            ...
            if (combatCard != null)
            {
                ...
                await CardPileCmd.Add(combatCard, deckPile);
            }
        }
        else
        {
            var deckPile = PileType.Deck.GetPile(Owner);
            await CardPileCmd.Add(generatedCard, deckPile);
            CardCmd.Preview(generatedCard);
        }
    }

    private static int RollWeighted(MegaCrit.Sts2.Core.Random.Rng rng)
    {
        int roll = rng.NextInt(TotalWeight);
        int cumulative = 0;
        foreach (var (caseId, weight) in RollTable)
        {
            cumulative += weight;
            if (roll < cumulative)
                return caseId;
        }
        return RollTable[^1].caseId;
    }

    private CardModel CreateCardFromTemplate(CardModel template, bool isInCombat)
    {
        return isInCombat
            ? Owner.Creature.CombatState.CreateCard(template, Owner)
            : Owner.RunState.CreateCard(template, Owner);
    }

    private async Task<bool> OfferOutOfCombatCardReward(
        IReadOnlyCollection<CardModel> templates,
        IReadOnlyCollection<CardPoolModel> cardPools,
        Func<CardModel, bool> filter,
        MegaCrit.Sts2.Core.Random.Rng rng,
        bool setCostToZero = false)
    {
        ...
        var cards = templates
            ...
            .Take(OutOfCombatRewardChoices)
            .Select(template => Owner.RunState.CreateCard(template, Owner))
            .ToList();

        if (setCostToZero)
        {
            foreach (var card in cards.Where(c => !c.EnergyCost.CostsX && c.EnergyCost.Canonical > 0))
                card.EnergyCost.UpgradeBy(-card.EnergyCost.Canonical);
        }

        if (cards.Count == 0) return false;

        var reward = new CardReward(cards, CardCreationSource.Other, Owner, options, null)
        {
            CanSkip = true,
            CanReroll = false
        };
    ...
    {
        var allAncient = Owner.UnlockState.CharacterCardPools
            .SelectMany(p => p.GetUnlockedCards(Owner.UnlockState, Owner.RunState.CardMultiplayerConstraint))
            .Where(c => c.Rarity == CardRarity.Ancient)
            .ToList();

        var template = allAncient.Count > 0
            ? rng.NextItem(allAncient)
            : ModelDb.GetById<CardModel>(new ModelId("CARD", WitchPowerCardIds[0]));
