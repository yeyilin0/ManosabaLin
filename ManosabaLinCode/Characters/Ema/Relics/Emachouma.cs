...
public sealed class Emachouma : ManosabaRelicTemplate, IEasyRightClickableRelic
{
    ...
    [
        "event:/ManosabaLin/sfx/relics/ema0",
        "event:/ManosabaLin/sfx/relics/ema1",
        "event:/ManosabaLin/sfx/relics/ema2",
        "event:/ManosabaLin/sfx/relics/ema3",
        "event:/ManosabaLin/sfx/relics/ema4",
        "event:/ManosabaLin/sfx/relics/ema5",
        "event:/ManosabaLin/sfx/relics/ema6"
    ];
    ...
    public async Task OnRightClick(PlayerChoiceContext choiceContext, RightClickContext clickContext)
    {
        ...
        var emalinPool = ModelDb.GetById<CardPoolModel>(ModelDb.GetId(typeof(EmalinCardPool)));
        var unlockedCards = emalinPool
        ...
        {
            case 0:
                var rareZeroCost = unlockedCards.Where(c => c.Rarity == CardRarity.Rare && c.EnergyCost.Canonical > 0 && !c.EnergyCost.CostsX).ToList();
                if (rareZeroCost.Count > 0) { if (!isInCombat) { await OfferOutOfCombatCardReward(rareZeroCost, [emalinPool], c => c.Rarity == CardRarity.Rare && c.EnergyCost.Canonical > 0 && !c.EnergyCost.CostsX && c.CanBeGeneratedInCombat, rng, setCostToZero: true); return; } generatedCard = CreateCardFromTemplate(rng.NextItem(rareZeroCost), isInCombat); generatedCard.EnergyCost.SetThisCombat(0); }
                break;
            case 1:
                var rareCards = unlockedCards.Where(c => c.Rarity == CardRarity.Rare).ToList();
                if (rareCards.Count > 0) { if (!isInCombat) { await OfferOutOfCombatCardReward(rareCards, [emalinPool], c => c.Rarity == CardRarity.Rare && c.CanBeGeneratedInCombat, rng); return; } generatedCard = CreateCardFromTemplate(rng.NextItem(rareCards), isInCombat); }
                break;
            case 2:
                var uncommonCards = unlockedCards.Where(c => c.Rarity == CardRarity.Uncommon).ToList();
                if (uncommonCards.Count > 0) { if (!isInCombat) { await OfferOutOfCombatCardReward(uncommonCards, [emalinPool], c => c.Rarity == CardRarity.Uncommon && c.CanBeGeneratedInCombat, rng); return; } generatedCard = CreateCardFromTemplate(rng.NextItem(uncommonCards), isInCombat); }
                break;
            case 3:
                var commonCards = unlockedCards.Where(c => c.Rarity == CardRarity.Common).ToList();
                if (commonCards.Count > 0) { if (!isInCombat) { await OfferOutOfCombatCardReward(commonCards, [emalinPool], c => c.Rarity == CardRarity.Common && c.CanBeGeneratedInCombat, rng); return; } generatedCard = CreateCardFromTemplate(rng.NextItem(commonCards), isInCombat); }
                break;
            case 4: return;
            case 5:
            ...
            case 6:
                ...
                if (otherPools.Count > 0) { if (!isInCombat) { var rewardPools = Owner.UnlockState.CharacterCardPools.Where(p => p != Owner.Character.CardPool).ToArray(); await OfferOutOfCombatCardReward(otherPools, rewardPools, c => c.Rarity != CardRarity.Ancient && c.Rarity != CardRarity.Basic && c.Rarity != CardRarity.Event && c.CanBeGeneratedInCombat, rng); return; } generatedCard = CreateCardFromTemplate(rng.NextItem(otherPools), isInCombat); }
                break;
        }
        ...
        if (isInCombat) { await CardPileCmd.AddGeneratedCardToCombat(generatedCard, PileType.Hand, Owner); if (combatCard != null) { var deckPile = PileType.Deck.GetPile(Owner); await CardPileCmd.Add(combatCard, deckPile); } }
